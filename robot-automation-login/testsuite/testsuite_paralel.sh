...
pabot \
  ...
  --argumentfile "${PROJECT_ROOT}/testsuite/testcase-list/api.txt" \
  --argumentfile "${PROJECT_ROOT}/testsuite/testcase-list/web.txt" \
  --argumentfile "${PROJECT_ROOT}/testsuite/testcase-list/ios.txt" \
  --argumentfile "${PROJECT_ROOT}/testsuite/testcase-list/android.txt" \
  "${TESTCASE_PATH}"

PDF_NAME="paralel_report_$(date +%Y-%m-%d_%H-%M-%S).pdf"
